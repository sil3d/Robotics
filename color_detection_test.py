...
# Import AprilTagPoseEstimator from april_tag_pose
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from april_tag_pose import AprilTagPoseEstimator, VideoCaptureThread, CAM_MATRIX, DIST_COEFFS

# AprilTag size in meters
APRILTAG_SIZE_M = 0.10  # 10 cm
ARUCO_TAG_SIZE  = APRILTAG_SIZE_M   # alias used by ArucoDetector (metres)
...
# ===========================================================================
# ARUCO DETECTOR  (v3 — detector created ONCE, faster dict, reuse gray)
# ===========================================================================

class ArucoDetector:
    """
    AprilTag / ArUco detector — RPi optimised.

    Key fix vs original code
    ------------------------
    The original created a new cv2.aruco.ArucoDetector() on EVERY frame
    (inside detect()), paying construction cost ~15×/sec.  Here it is
    built once in __init__ and reused.
    """
    ...
    def __init__(self):
        ...
        # Speed-tuned parameters for RPi 4B
        params.minMarkerPerimeterRate                 = 0.03   # add this line
        ...
        params.errorCorrectionRate                    = 1.5

        # Build ONCE — this is the expensive part
        self.detector = cv2.aruco.ArucoDetector(aruco_dict, params)

        # 3-D object points for pose estimation (tag corners in tag frame)
        half = ARUCO_TAG_SIZE / 2.0
    ...
    def detect(self, gray_small):
        """
        Detect ArUco markers in a grayscale image (already at process res).
        Returns list of dicts with id, tvec, rvec, distance, corners.

        Caller should pass the SAME resized gray frame used by ColorDetector
        to avoid an extra resize + cvtColor.
        """
        ...
        for i, tag_id in enumerate(ids.flatten()):
            ok, rvec, tvec = cv2.solvePnP(
                self.obj_pts, corners[i], CAM_MATRIX, DIST_COEFFS)
            if ok:
                results.append({
                    'id':       int(tag_id),
                    'tvec':     tvec.flatten(),   # [x, y, z] in metres
                    'rvec':     rvec.flatten(),
                    'distance': round(float(tvec.flatten()[2]), 3),
                    'corners':  corners[i],
                })
        return results
...
def main():
    ...
    print("\n" + "=" * 56)
    print("  COLOR DETECTION v3 - RPi 4B OPTIMIZED")
    print("=" * 56)
    ...
    print(f"  ArUco dict   : DICT_4X4_250")
    print("=" * 56)
    print("  Q=Quit  S=Save  M=Masks  A=ArUco toggle")
    print("=" * 56 + "\n")
    ...
    while True:
        ...
        if show_aruco:
            ...
            if state == RobotState.SEARCHING_BOX:
                ...
                station_found = False
                for tag in tags:
                    if tag['id'] == target_station_id:
                        distance_m = tag['distance']
                        station_found = True
                        # Draw extra info
                ...
                drop_counter += 1
                if drop_counter > 30:  # ~2 seconds at 15 fps
                    drop_counter = 0
